...
function PersistSpray()
    ...
    if rayEndCoords and rayNormal then
        ...
        -- Replace the old CancellableProgress with the new progress bar
        local isFinished = exports["erp_progressbar"]:taskBar({ 
            length = Config.SPRAY_PROGRESSBAR_DURATION, 
            text =  'Spraying', 
            runcheck = false, 
            ignoreclear = true,
            keepweapon = false,
            vehicle = false,
            distcheck = 1.0,
            animation = { dict = 'anim@amb@business@weed@weed_inspecting_lo_med_hi@', anim = 'weed_spraybottle_stand_spraying_01_inspector', stuck = true }
        }) == 100
