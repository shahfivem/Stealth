...
RegisterNUICallback("sendFine", function(data, cb)
    local stateId, fine, incidentId = data.stateId, data.fine, data.incidentId

    -- Resolves the player source ID from the State ID
    local p = promise.new()
    ...
    if fine > 0 and targetSourceId then
        if Config.BillVariation then
            -- Uses QB-Core removeMoney Functions
            TriggerServerEvent("mdt:server:removeMoney", stateId, fine, incidentId)
        else
            -- Uses QB-Core /bill command
            ExecuteCommand(('bill %s %s'):format(targetSourceId, fine))
            TriggerServerEvent("mdt:server:giveCitationItem", stateId, fine, incidentId)
        end
    else
        TriggerEvent('QBCore:Notify', "Could not find the player with the given State ID.", "error")
    end
end)
...
GetClosestNPC = function(source, data)

    if not data then return end

    local pos = data.pos or GetEntityCoords( PlayerPedId() )
    
    local minDist = data.minDist or 0.0
    local maxDist = data.maxDist or 30.0

    local type = data.type or 'nothing'
    local ignoredPed = data.object
    maxDist = maxDist * (math.random(10, 14) / 10)
    ...
    for i=1, #allPeds do
        ...
        if DoesEntityExist(ped) then
            if ped ~= ignoredPed and ped ~= GetPlayerPed(PlayerPedId()) then
                local isValid = not IsPedBeingStunned(ped, 0) and not IsPedDeadOrDying(ped, true) and IsPedHuman(ped) and not IsPedInAnyVehicle(ped, false) and not IsPedShooting(ped) and not IsPedAPlayer(ped) and NetworkGetEntityIsNetworked(ped) and GetEntityPopulationType(ped) ~= 7 and not IsPedAimingFromCover(ped)
                if isValid then
                    local dist = #(GetEntityCoords(ped) - pos)
                    if dist > minDist and dist < maxDist then
                        if type == 'gunshot' then -- This is a special type because we would need to check if the ped is even alert and not aiming from cover
                            if (GetPedAlertness(ped) > 0) and not IsPedAimingFromCover(ped) then
                                TaskUseMobilePhoneTimed(ped, 5000)
                                return ped
                            end
                        elseif HasEntityClearLosToEntity(GetPlayerPed(PlayerPedId()), ped, 17) then
                            TaskUseMobilePhoneTimed(ped, 5000)
                            return ped
                        end
                    end
