...
local function onStoreEnter(data)
    ...
    if jobName == (store.job or store.gang) then
        ...
        if currentZone.name == "clothing" then
            text = string.format(_L("textUI.clothing"), Config.ClothingCost)
        elseif currentZone.name == "barber" then
            text = string.format(_L("textUI.barber"), Config.BarberCost)
        elseif currentZone.name == "tattoo" then
            text = string.format(_L("textUI.tattoo"), Config.TattooCost)
        elseif currentZone.name == "surgeon" then
            text = string.format(_L("textUI.surgeon"), Config.SurgeonCost)
        end

        if text then
            lib.showTextUI(prefix .. text, nil, Config.TextUIOptions)
