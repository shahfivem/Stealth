...
function Handler:fixTire(vehicle, coords)
    ...
    if found then
        ...
        if exports["erp_progressbar"]:taskBar(Progress['tirekit']) == 100 then
            success = true
...
function Handler:fixVehicle(vehicle, coords, fixtype)
    ...
    if fixtype == 'smallkit' and engine < 500 or fixtype == 'bigkit' and engine < 1000 then
        if #(coords - offset) < 2.0 then
            ...
            if exports["erp_progressbar"]:taskBar(Progress[fixtype]) == 100 then
                success = true
            end
        ...
        lib.notify({
            text = 'Cannot repair vehicle any further',
            type = 'error'
        })
    end
...
function Handler:basicwash()
    ...
    if exports["erp_progressbar"]:taskBar(Progress['cleankit']) == 100 then
        success = true
