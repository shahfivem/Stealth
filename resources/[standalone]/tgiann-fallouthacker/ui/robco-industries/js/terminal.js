...
{
	var kelimeler = []
	$.get('../ui/robco-industries/ajax/wordlist.txt',{},function(content){
		kelimeler = content.split(' ');
		var Nwords = []
		for (let index = 0; index < 12; index++) {
			Nwords.push(kelimeler[Math.floor(Math.random() * kelimeler.length)])
		}
		Words = Nwords
		Correct = Words[0];
		Words = Shuffle(Words);
...
{
	$("#terminal").html('<div id="terminal-interior"><button type="button" id="hatali" class="x-button">x</button><div id="info"></div><div id="attempts"></div><div id="column1" class="column pointers"></div><div id="column2" class="column words"></div><div id="column3" class="column pointers"></div><div id="column4" class="column words"></div><div id="output"></div><div id="console">></div></div>');
	document.getElementById("hatali").onclick = hatali;
...
TogglePower = function(health, hackStatus)
{
	if (Power == "on")
	{
		...
		$("#terminal").html("");
		$.post('https://tgiann-falloutHacker/menuClose', JSON.stringify({success: hackStatus}));
		if (Sound)
			$("#poweroff")[0].play();
	}
	else
...
HandleBraces = function(DudCap)
{
	...
	else
	{
		...
		//$.post('https://tgiann-falloutHacker/bildirim', JSON.stringify({}));
		RemoveDud();
	}
...
Failure = function()
{
	...
	},
	{
		...
		complete : function()
		{
			$("#terminal").html("<div id='canvas'></div><div id='adminalert'><div class='character-hover alert-text' id = 'hatali'>TERMINAL LOCKED<br />PLEASE CONTACT AN ADMINISTRATOR</div></div>");
...
Success = function()
{
	...
	},
	{
		...
		complete : function()
		{
			...
			document.getElementById("proceed").onclick = success;
