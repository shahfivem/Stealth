...
local Webhooks = {
    [1] = GetConvar('R14_fivemerr_api_key_1', ''),
}

for k, v in pairs(Webhooks) do
    if v == '' then
        print(('Warning: API Key for Webhook %s is missing! Please check your server.cfg.'):format(k))
    end
end
...
RegisterNetEvent('evidence:server:uploadphotoDiscord', function(uploadPath, photo)
...
RegisterNetEvent('evidence:server:uploadphoto', function(uploadPath, photo)
    if uploadPath == 1 or photo.uploadPath == 1 then
        print(("%s attempted to upload image to local storage via %s??? %s "):format(source,
            GetInvokingResource() or 'client', json.encode(photo)))
        return
    end
    local imageSpace = Config.Camera[uploadPath].Path
    local imageSpaceSize = #ImageSpaces[imageSpace]
    ...
    local headers = {
        ['Authorization'] = Webhooks[uploadPath],
        ['Content-Type'] = 'application/json'
    }
    PerformHttpRequest('https://api.fivemerr.com/v1/media/images', function(err, text, headers)
        if err == 200 then
    ...
    end, 'POST', json.encode(formData), headers)
end)
