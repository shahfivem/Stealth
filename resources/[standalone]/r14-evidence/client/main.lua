...
RegisterNetEvent('evidence:client:SetStatus', function(statusId, time, abv, customText)
    ...
    if customText and not Config.StatusList[statusId] then
        Config.StatusList[statusId] = customText
    end
...
RegisterNetEvent('evidence:client:drugtest', function(data)
    ...
    local d = exports.erp_progressbar:taskBar({
        text = 'Administering Multipanel Oral Drug Test...',
        length = 5000,
        animation = {
            dict = nil,
            anim = nil,
...
RegisterNetEvent('evidence:client:fingerprint', function(data)
    ...
    if d == 100 then -- Progress completed
        ClearPedTasks(ped)
        ...
        if #(GetEntityCoords(ped) - GetEntityCoords(data.entity)) < 4 then
            TriggerServerEvent('evidence:server:fingerprint', {
                player = GetPlayerServerId(NetworkGetPlayerIndexFromPed(data.entity)),
                street = GetStreet()
            })
        else
            Config.Functions.Notify(Config.Notification.Evading, 'error')
        ...
        ClearPedTasks(ped) -- Progress canceled
    end
end)
...
RegisterNetEvent('evidence:client:wipearea', function(data)
    ...
    local d = exports.erp_progressbar:taskBar({
        text = data?.clean and 'Cleaning area...' or 'Wiping down area...',
        length = wipeareaTime,
        animation = {
            dict = "timetable@floyd@clean_kitchen@idle_a",
            anim = 'idle_b',
            stuck = false
        }
    })
    ...
    if d == 100 then -- Progress completed
        ...
        TriggerServerEvent('evidence:server:wipearea', {
            slot = data?.slot,
            clean = data?.clean,
            plate = data?.seat and Config.Functions.GetPlate(curveh),
            seat = data?.seat
        })
...
RegisterNetEvent('evidence:client:wipeweapon', function(data)
    local d = exports.erp_progressbar:taskBar({
        text = data.clean and 'Cleaning weapon...' or 'Wiping down weapon...',
        length = 4000,
        animation = {
            dict = "mp_arresting",
            anim = "a_uncuff",
...
RegisterNetEvent('evidence:client:unbagev', function(slot)
    if Config.Context.Ox and lib then
        lib.hideContext(lib.getOpenContextMenu())
    end
    ...
    local d = exports.erp_progressbar:taskBar({
        text = "Unbagging evidence...",
        length = ActiveLEO and 10000 or 60000,
        animation = {
            dict = nil,
            anim = nil,
            stuck = false
...
RegisterNetEvent('evidence:client:collectlatent', function(data)
    ...
    local d = exports.erp_progressbar:taskBar({
        text = "Collecting latent evidence..",
        length = 3000,
        animation = {
            dict = "nmt_3_rcm-10",
    ...
    if d == 100 then -- Progress completed
        local info = {
            ...
            type = data.evtype,
            street = ('%s %s %s'):format(
                data.item[inventoryMetadata].item.label,
                data.item[inventoryMetadata].tracking,
                data.item[inventoryMetadata].serie or data.item[inventoryMetadata].serial or ''
            ),
        }
