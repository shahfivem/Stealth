...
function progressBar(data) -- feel free to implement your own progress bar system here
    ...
    elseif config.progressBar == 'ox_bar' then
        if exports["shah-progressbar"]:taskBar({ 
            length = data.duration,  -- 5 seconds
            text = data.label,  -- Custom message
            runcheck = true,
            keepweapon = false,
            distcheck = 2.0,  -- Check if the player moves more than 2 meters
            animation = { dict = data.anim.dict, anim = data.anim.clip, stuck = true }  -- Example animation
        })
        then
            data.onSuccess()
        else
