...
local function isVehAllowed()
    if GetVehicleClass(cache.vehicle) ~= 18 or cache.seat ~= -1 or IsPedInAnyHeli(cache.vehicle) or IsPedInAnyPlane(cache.vehicle) then
        return false
    end
...
local policeLights = lib.addKeybind({
    ...
    onPressed = function()
        ...
        local newLightsState = not curMode
        state:set('lightsOn', newLightsState, true)

        -- Play sound based on the new lights state
        if newLightsState then
            TriggerEvent('erp-sounds:PlayOnOne', 'On', 0.5)  -- Play sound for lights on
        else
            TriggerEvent('erp-sounds:PlayOnOne', 'Off', 0.5) -- Play sound for lights off
        end
