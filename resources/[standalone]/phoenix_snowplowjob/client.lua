...
startsnowplow = function(veh, vehcoords)
    ...
    -- Set a unique plate for the snowplow
    local plate = "SNOW"
    SetVehicleNumberPlateText(globalvehicle, plate)
    
    -- Set the player as the owner of the vehicle keys
    TriggerEvent('vehiclekeys:client:SetOwner', QBCore.Functions.GetPlate(globalvehicle))
    
    -- Customize the snowplow if it's a specific vehicle model
    if veh == 'biff' then
        SetVehicleExtra(globalvehicle, 10, true)
        SetVehicleExtra(globalvehicle, 11, true)
    end
    
    -- Notify the player about the start of the job
    Config.MSG(Translation[Config.Locale]['info_on_start'])
    
    -- Spawn props if configured
