...
window.addEventListener('message', (event) => {
    ...
    switch(data.type) {
        case 'charid':
            MakePopup(data.message, 'charid', data.min ?? 4, data.max ?? 5);
            break;
        case 'text':
            MakePopup(data.message, 'text', data.min ?? 5, data.max ?? 255);
            break;
        case 'number':
            MakePopup(data.message, 'number', data.min ?? 1, data.max ?? 255);
            break;
        case 'date':
            MakePopup(data.message, 'date', 10, 10);
            break;
        case 'name':
            MakePopup(data.message, 'text', data.min ?? 1, data.max ?? 32);
            break;
        case 'confirm':
...
function HideInputUI() {
    $.post('https://prp-common/Input.ReturnData', JSON.stringify({
        input: null
    }));
