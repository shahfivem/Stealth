...
RegisterNetEvent('getbankamount', function()
    local src = source -- The player source calling the event
    local player = exports.qbx_core:GetPlayer(src) -- Fetch player data

    if player and player.PlayerData and player.PlayerData.money then
        local bankAmount = player.PlayerData.money.bank -- Get bank balance

        -- Send the bank balance back to the client
        TriggerClientEvent('sendbankammount', src, bankAmount)
    else
        -- Notify the client if there was an issue retrieving player data
        TriggerClientEvent('ox_lib:notify', src, {
            text = 'Unable to fetch your bank account details.',
            type = 'error',
            length = 5000
        })
    end
