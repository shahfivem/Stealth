...
RegisterNetEvent('brazzers-fakeplates:server:usePlate', function(vehNetID, vehPlate, newPlate, hasKeys)
    ...
    if isFakePlateOnVehicle(vehPlate) then return lib.notify({title = 'Notification', description = 'Already has a plate', type = 'error'}) end
    if not isVehicleOwned(vehPlate) then
        if not hasKeys then return lib.notify({title = 'Notification', description = 'You do not own this vehicle', type = 'error'}) end
...
RegisterNetEvent('brazzers-fakeplates:server:removePlate', function(vehNetID, vehPlate, hasKeys)
    ...
    if not isFakePlateOnVehicle(vehPlate) then return lib.notify(src, {title = 'Notification', description = 'No plate found', type = 'error'}) end
