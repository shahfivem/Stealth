...
local function canAffordItem(inv, currency, price)
    ...
    print("Checking if player can afford the item...")
    print("Currency:", currency, "Price:", price)

    local canAfford = price >= 0

    if currency == 'bank' then
        local player = exports.qbx_core:GetPlayer(source) 
        if not player then
            print("Player not found!")
            return false, false, { type = 'error', description = locale('player_not_found') }
        end

        local bankAccount = exports.qbx_core:GetMoney(source, 'bank')
        if not bankAccount then
            print("Bank account not found!")
            return false, false, { type = 'error', description = locale('no_bank_account') }
        end

        print("Bank balance:", exports.qbx_core:GetMoney(source, 'bank'))
        canAfford = exports.qbx_core:GetMoney(source, 'bank') >= price
    else
        canAfford = Inventory.GetItem(inv, currency, false, true) >= price
    end

    local formattedPrice = price
    local formattedCurrency = (currency == 'money' and locale('$'))
    if not formattedCurrency then
        formattedCurrency = formattedPrice 
    end

    return canAfford or {
        type = 'error',
        description = locale('cannot_afford', ('%s%s'):format(formattedCurrency, (currency == 'money' or ' '..Items(currency).label)))
    }
end
...
local function removeCurrency(inv, currency, price)
    ...
    exports.qbx_core:RemoveMoney(source, 'bank', price, "Purchased Item(s) from Shop")
end
	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem?.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				...
				local message = locale('purchased_for', count, metadata?.label or fromItem.label, (currency == 'bank' and locale('$') or math.groupdigits(price)), (currency == 'bank' and math.groupdigits(price) or ' '..Items(currency).label))
