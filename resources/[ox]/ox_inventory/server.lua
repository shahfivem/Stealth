...
lib.addCommand('giveweapon', {
    help = 'Gives a weapon to a player with specified ammo',
    params = {
        { name = 'target', type = 'playerId', help = 'Player ID to receive the weapon' },
        { name = 'weapon', type = 'string', help = 'Weapon name (without WEAPON_ prefix)' },
        { name = 'ammo', type = 'number', help = 'Amount of ammo to give', optional = true },
    },
    restricted = 'group.admin', -- Adjust based on your permissions
}, function(source, args)
    local inventory = Inventory(args.target) --[[@as OxInventory]]
    local weaponName = 'WEAPON_' .. string.upper(args.weapon)
    local ammo = args.ammo or 0

    -- Add weapon to the player's inventory
    local success, response = Inventory.AddItem(inventory, weaponName, 1, { ammo = ammo })

    if not success then
        return print(('Failed to give weapon %s to player %s (%s)'):format(weaponName, args.target, response))
    end

    print(('Weapon %s with %d ammo given to player %s'):format(weaponName, ammo, args.target))
end)
