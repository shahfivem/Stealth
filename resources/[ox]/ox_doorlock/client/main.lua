...
RegisterNetEvent('dooranim', function()
	...
	if GetPedType(plyPed) == 28 then return end
	...
	if not cache.vehicle then
		...
		TaskPlayAnim(plyPed, dict, 'exit', 8.0, 1.0, -1, 16, 0, false, false, false)
	end

	PlaySoundFromEntity(-1, "Keycard_Success", plyPed, "DLC_HEISTS_BIOLAB_FINALE_SOUNDS", true, 5.0)

	Wait(850)
	StopAnimTask(plyPed, dict, 'exit', 2.0)
end)
	...
	while true do
		...
		if ClosestDoor and ClosestDoor.distance < ClosestDoor.maxDistance then
			if Config.DrawTextUI and not ClosestDoor.hideUi and ClosestDoor.state ~= showUI then
				local text = 'Press E'
				local additionalText = ClosestDoor.state == 0 and 'To lock this door' or 'to unlock this door'
				lib.showTextUI(text, additionalText)
