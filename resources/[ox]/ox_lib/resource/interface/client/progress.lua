...
function lib.progressBar(data)
    ...
    if not interruptProgress(data) then
        ...
        local disableOptions = {
            ...
            movement = data.disable and data.disable.movement or false,
            car = data.disable and data.disable.car or false,
            combat = data.disable and data.disable.combat or false,
        }

        -- Prepare task bar data
        local taskBarData = {
            length = data.duration or 5000, -- Progress duration in ms
            text = data.label or "Processing...",
            disable = disableOptions,
            animation = data.anim, -- Animation configuration if any
            preventCancel = data.canCancel == false, -- Prevent cancel if specified
            keepweapon = true
        }

        -- Set progress as active
        progress = true

        -- Trigger the task bar
        local result = exports.erp_progressbar:taskBar(taskBarData)

        -- Handle task bar result
        if result == 100 then
            -- Progress completed
