...
RegisterNetEvent('qbx_recyclejob:client:target:pickupPackage', function()
    ...
    if exports.erp_progressbar:taskBar({
            length = config.pickupActionDuration,  -- Duration in milliseconds
            text = locale("text.picking_up_the_package"),  -- Display text
...
RegisterNetEvent('qbx_recyclejob:client:target:dropPackage', function()
    ...
    if exports.erp_progressbar:taskBar({
            length = config.deliveryActionDuration,  -- Duration in milliseconds
            text = locale("text.unpacking_the_package"),  -- Display text
            canCancel = true,  -- Allows canceling (equivalent to `preventCancel = false`)
            showTime = false,  -- Show remaining time (equivalent to `showTime = true`)
        }) == 100 then
        StopAnimTask(cache.ped, 'mp_car_bomb', 'car_bomb_mechanic', 1.0)
