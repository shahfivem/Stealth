...
if GetConvar('qbx:enablebridge', 'true') == 'true' then
    ...
    QBCore.Functions.CreateCallback('police:IsPlateFlagged', function(_, cb, plate)
        lib.print.warn(GetInvokingResource(),
            'invoked deprecated callback police:IsPlateFlagged. Use qbx_police:server:isPlateFlagged instead.')
...
RegisterNetEvent('police:server:EscortPlayer', function(escortSrc)
    ...
    if (player.PlayerData.job.type == 'leo' or player.PlayerData.job.type == 'ems') or
        ...
        if escortPlayer.PlayerData.metadata.isdead or escortPlayer.PlayerData.metadata.inlaststand then
            exports["qbx_medical"]:PlayLastStandAnimation(true, GetPlayerPed(escortPlayer.PlayerData.source))
        end
    else
...
RegisterNetEvent('police:server:FlaggedPlateTriggered', function(radar, plate, street)
    ...
    for i = 1, #players do
        if IsLeoAndOnDuty(players[i]) then
            local alertData = { title = locale('info.new_call'), coords = coords, description = locale(
