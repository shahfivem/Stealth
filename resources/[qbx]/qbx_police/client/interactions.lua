...
RegisterNetEvent('police:client:RobPlayer', function()
    ...
    if not (IsEntityPlayingAnim(playerPed, 'missminuteman_1ig_2', 'handsup_base', 3)
        ...
        or IsEntityPlayingAnim(playerPed, 'missarmenian2', 'drunk_loop', 3)
        or isTargetDead(playerId))
    then
        return exports.qbx_core:Notify(locale('error.no_rob'), 'error')
    end

    if GetPedType(playerPed) == 28 then
        exports.erp_notifications:SendAlert('inform', 'Unable to search animal')
        return
    end
    ...
    local finished = exports.erp_progressbar:taskBar({
        ...
        text = "Searching",
        distcheck = 2.0,
        animation = { dict = 'missexile3', anim = 'ex03_dingy_search_case_a_michael' }
