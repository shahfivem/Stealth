...
local function checkIn(hospitalName)
    print("Checking in to hospital:", hospitalName) -- Debugging

    local canCheckIn = lib.callback.await('qbx_ambulancejob:server:canCheckIn', false, hospitalName)
    if not canCheckIn then
        print("Check-in blocked, cannot check in.") -- Debugging
        exports.qbx_core:Notify(locale('error.cannot_check_in'), 'error')
        return
    end

    -- Show progress bar for checking in
    if exports.erp_progressbar:taskBar({
        length = 2000,  -- Duration in milliseconds
        text = locale('progress.checking_in'),  -- Display text
