...
lib.callback.register('qbx_divegear:client:fillTank', function()
    ...
    if lib.progressBar({
        duration = config.refillTankTimeMs,  -- Duration in milliseconds
        label = locale('info.filling_air'),  -- Display text
        canCancel = true,  -- Allow canceling (equivalent to `preventCancel = false`)
        showTime = true,  -- Show remaining time
    
        anim = {
            dict = 'clothingshirt',  -- Animation dictionary
            clip = 'try_shirt_positive_d',  -- Animation clip
            blendIn = 8.0  -- Custom blend-in time (optional if supported)
        },
    })
     then
...
local function takeOffSuit()
    local success = lib.progressBar({
        duration = config.takeOffSuitTimeMs,  -- Duration in milliseconds
        label = locale('info.pullout_suit'),  -- Display text
        canCancel = true,  -- Allow canceling (equivalent to `preventCancel = false`)
        showTime = true,  -- Show remaining time
        ...
        anim = {
            dict = 'clothingshirt',  -- Animation dictionary
            clip = 'try_shirt_positive_d',  -- Animation clip
            blendIn = 8.0  -- Blend-in time, if supported
        }
    })    
    if success then
        SetEnableScuba(cache.ped, false)
