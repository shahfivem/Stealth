...
local function fireEmployee(employeeCitizenId, boss, groupName, groupType)
    -- Get employee from citizen ID or offline data
    local employee = exports.qbx_core:GetPlayerByCitizenId(employeeCitizenId) 
        or exports.qbx_core:GetOfflinePlayer(employeeCitizenId)

    -- Prevent boss from firing themselves
    if employee.PlayerData.citizenid == boss.PlayerData.citizenid then
        local message = groupType == 'gang' and locale('error.kick_yourself') 
    ...
    -- Verify grades for firing
    local employeeGrade = groupType == 'job' and employee.PlayerData.jobs?[groupName] 
        or employee.PlayerData.gangs?[groupName]
    local bossGrade = groupType == 'job' and boss.PlayerData.jobs?[groupName] 
        or boss.PlayerData.gangs?[groupName]
    ...
    -- Remove employee from the group (job or gang)
    local success, errorResult
    if groupType == 'job' then
        success, errorResult = exports.qbx_core:RemovePlayerFromJob(employee.PlayerData.citizenid, groupName)
    else
        success, errorResult = exports.qbx_core:RemovePlayerFromGang(employee.PlayerData.citizenid, groupName)
    end

    -- Ensure operation succeeded
    assert(success, errorResult?.message)

    -- Notify the employee if they are online
    if not employee.Offline then
        local message = groupType == 'gang' and locale('error.you_gang_fired', GANGS[groupName].label) 
            or locale('error.you_job_fired', JOBS[groupName].label)
        exports.qbx_core:Notify(employee.PlayerData.source, message, 'error')

        -- Trigger event to remove multi-job data (auto-fire logic)
        TriggerEvent('ps-multijob:server:removeJob', employee.PlayerData.citizenid)
    end

    -- Notify the boss about successful firing
    exports.qbx_core:Notify(boss.PlayerData.source, "Employee fired!", "success")

    -- Trigger the same event again for consistency (as requested)
    TriggerEvent('ps-multijob:server:removeJob', employee.PlayerData.citizenid)
