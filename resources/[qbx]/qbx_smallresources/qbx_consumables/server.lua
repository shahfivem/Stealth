...
---Set player's armor state to 'amount'
---@param source number
---@param amount number amount of armor to set
local function setArmor(source, amount)
    amount = lib.math.clamp(amount, 0, 100)
    local player = exports.qbx_core:GetPlayer(source)
    if player then
        player.Functions.SetMetaData('armor', amount)
    end
    SetPedArmour(GetPlayerPed(source), amount)
end

---Increment player's armor state by 'amount'
---@param source number
---@param amount number amount of armor to add
local function addArmor(source, amount)
    local currentArmor = GetPedArmour(GetPlayerPed(source))
    setArmor(source, currentArmor + amount)
end
...
---@param max number upper limit for random generation
exports('relieveStress', function(source, min, max)
...
for drink, params in pairs(config.consumables.drink) do
    exports.qbx_core:CreateUseableItem(drink, function(source, item)
        ...
        -- Add armor if the drink has an armor value
        if params.armor then
            addArmor(source, params.armor)
        end
    end)
end
