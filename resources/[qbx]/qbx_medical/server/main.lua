...
-- ---@param player table|number
-- local function emsrevivePlayer(player)
--     TriggerClientEvent('qbx_medical:client:emsplayerRevived', player --[[@as number]])
-- end

-- exports('emsRevive', emsrevivePlayer)
...
lib.addCommand('revive', {
	...
	if not args.id then args.id = source
		exports.qbx_core:Notify(source, "You didn't provide a ID, ill just revive you", 'inform')
	 end
...
lib.addCommand('heal', {
	...
	if not args.id then args.id = source
		exports.qbx_core:Notify(source, "You didn't provide a ID, ill just heal you", 'inform')
	 end
	local player = exports.qbx_core:GetPlayer(tonumber(args.id))
	if not player then
		exports.qbx_core:Notify(source, locale('error.not_online'), 'error')
