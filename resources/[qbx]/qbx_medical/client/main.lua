...
RegisterNetEvent('qbx_medical:client:playerRevived', function()
    if DeathState ~= sharedConfig.deathState.ALIVE then
        ...
        local dict = 'anim@scripted@player@freemode@drugged_respawn@'
        lib.requestAnimDict(dict)
        TaskPlayAnim(cache.ped, dict, "drugged_respawn_player_03", 8.0, -8, -1, 1, 0, false, false, false)
        Wait((GetAnimDuration(dict, "drugged_respawn_player_03") * 1000) - 4500)
...
-- RegisterNetEvent('qbx_medical:client:emsplayerRevived', function()
--     if DeathState ~= sharedConfig.deathState.ALIVE then
--         local pos = GetEntityCoords(cache.ped, true)
--         NetworkResurrectLocalPlayer(pos.x, pos.y, pos.z, GetEntityHeading(cache.ped), true, false)
--         SetDeathState(sharedConfig.deathState.ALIVE)
