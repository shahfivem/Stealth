...
AddEventHandler('erp:dances:dance', function(pDance)
        ...
        if pDance == -1 then
			...
			lib.notify({
                label = 'Random dance selected: '..pDancem,
                duration = 5000
            })
        end
		...
		if pDance > noAnimations or pDance <= 0 then
            lib.notify({
                label = 'There are only '..noAnimations..' dances, select a number inbetween.',
                duration = 5000
            })
            return
        end
		...
		TaskPlayAnim(plyPed, animations[pDance].dict, animations[pDance].anim, 3.0, 3.0, -1, 1, 0, 0, 0, 0)
		currDance = pDance 
end)

TriggerEvent('chat:addSuggestion', '/dance', 'Do a dance!', {
    { name="ID (Optional)", help="You can leave this empty for random choice, enter 'c' for cancel or input a number" },
})
...
RegisterCommand("dance", function(source, args, rawCommand)
    ...
    if args[1] == nil or args[1] == '' then
        ...
        local dance = math.random(1, #animations)
        TriggerEvent('erp:dances:dance', tonumber(dance))

        print('Doing dance #'..dance)
        lib.notify({
            label = 'Doing dance #'..dance,
            duration = 5000
        })
    
    elseif args[1] == 'cancel' or args[1] == 'c' then
